...
/* ============================================================================
   Description : VINAX Driver, VxWorks part
   ========================================================================= */
...
#ifdef MEI_VXWORKS_PROC_FS
#include "drv_mei_cpe_vxworks_proc.h"
#endif
...
#define MEI_STATIC
#else
#define MEI_STATIC   LOCAL
#endif


/* ============================================================================
   Local variables (vxWorks)
   ========================================================================= */
...
/* ============================================================================
   I/O VINAX driver functions (vxWorks) - declaration
   ========================================================================= */

MEI_STATIC IFX_int32_t MEI_CpeDevOpen(
                              MEI_DEV_T *pMeiDev,
                              IFX_int8_t  *pAnnex,
                              IFX_int32_t flags);

MEI_STATIC IFX_int32_t MEI_Close(
                              MEI_DYN_CNTRL_T *pMeiDynCntrl);

MEI_STATIC IFX_int32_t MEI_Write(
                              MEI_DYN_CNTRL_T *pMeiDynCntrl,
                              IFX_uint8_t       *pSrc,
                              IFX_int32_t       nLength);

MEI_STATIC IFX_int32_t MEI_Read(
                              MEI_DYN_CNTRL_T *pMeiDynCntrl,
                              IFX_uint8_t       *pDst,
                              IFX_int32_t       nLength);
...
MEI_STATIC STATUS MEI_IfxIntConnect(
                              VOIDFUNCPTR    *pIntVector,
                              VOIDFUNCPTR    pISRRoutine,
                              int            ISRParams);

MEI_STATIC void MEI_InterruptVxWorks(
                              int ISRParams);


/* ============================================================================
   I/O MEI X (Control) driver functions (vxWorks) - declaration
   ========================================================================= */
...
MEI_STATIC IFX_int32_t MEI_CloseDfeX(MEIX_CNTRL_T *pMeiXCntrl);


/* ============================================================================
   Local function (vxWorks) - declarations
   ========================================================================= */
...
/* ============================================================================
   VINAX driver interrupt functions (vxWorks) - wrapping
   ========================================================================= */

MEI_STATIC int MEI_IntSetupLocked = 0;

/* function ptr to the used intConnect function */
MEI_STATIC MEI_IntConnect_WrapVxW_t MEI_IntConnect_WrapVxW = intConnect;

/* function ptr to the used Interrupt Enable Routine */
MEI_STATIC MEI_IntEnable_WrapVxW_t MEI_IntEnable_WrapVxW = intEnable;

/* function ptr to the used Interrupt Enable Routine */
MEI_STATIC MEI_IntDisable_WrapVxW_t MEI_IntDisable_WrapVxW = intDisable;


/* ============================================================================
   I/O driver functions (vxWorks) - definition
   ========================================================================= */
                              ...
                              IFX_int32_t       nArgument)
{
   ...
   if (pMeiDynCntrl == NULL)
   {
      ...
      switch (nCmd)
      {
         case FIOSELECT:
         case FIOUNSELECT:
            /* select call don't have a VINAX driver argument struct */
            break;
         default:
   ...
   */
   ...
   switch (nCmd)
   {
      case FIOSELECT:
         {
            ...
            MEI_DRVOS_SelectQueueAddTask(
                        (MEI_DRVOS_select_OSArg_t*) nArgument,
                        (MEI_DRVOS_selectQueue_t*)  &(pMeiDev->selNfcWakeupList),
                        (MEI_DRVOS_selectTable_t*)  IFX_NULL);


            /* check if there is already data in the NFC receive buffer */
      ...
      case FIOUNSELECT:
         selNodeDelete ( &( pMeiDev->selNfcWakeupList ), ( SEL_WAKEUP_NODE * ) nArgument );
         break;
...
/**
   ...
   - do the standard VxWorks intConnect
...
\remark
   The function wrapps the normal VxWorks intConnect function and additional
                                 ...
                                 int ISRParams)
{
   ...
   */
   if ( (ret = MEI_IntConnect_WrapVxW( pIntVector, pISRRoutine, ISRParams)) == ERROR )
   {
...
*/
MEI_STATIC void MEI_InterruptVxWorks(int ISRParams)
{
   ...
   if (pMeiXCntrlList->IRQ_Protection <= 0)
   {
      ...
      PRN_ERR_INT_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR, (MEI_DRV_CRLF));
      MEI_IntDisable_WrapVxW(pMeiXCntrlList->IRQ_Num);
...
/* ============================================================================
   I/O VINAX X (Control) driver functions (vxWorks) - definition
   ========================================================================= */
...
/* ============================================================================
   Local function definitions (vxWorks)
   ========================================================================= */
...
MEI_STATIC IFX_int32_t MEI_DevFreeAll(void)
{
   ...
   allFree = 0;
   for (entityNum=0; entityNum < MEI_MAX_DFEX_ENTITIES; entityNum++)
   {
                                 ...
                                 IFX_uint8_t *pDevNumLast)
{
   ...
   for (deviceNum=0; deviceNum < MEI_MAX_DFE_CHAN_DEVICES; deviceNum++)
   {
           ...
           ((pMeiDev = (MEIX_Cntrl[entity])->MeiDevice[rel_ch]) != NULL) )
      {
         ...
         PRN_DBG_USR( MEI_DRV, MEI_DRV_PRN_LEVEL_NORMAL, deviceNum,
                ("MEI_DRV: iosDevAdd - device <%s>" MEI_DRV_CRLF, buf));

         /* Add device to the device list */
         if (iosDevAdd(&(pMeiDev->DevHdr), buf, drvNum) == ERROR)
         {
            PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR,
   ...
   *pDevNumLast = MEI_MAX_DFE_CHAN_DEVICES;
...
MEI_STATIC IFX_void_t MEI_DevDeleteAll(IFX_uint8_t devNumLast)
{
   ...
   for (deviceNum=0; deviceNum < devNumLast; deviceNum++)
   {
           ...
           ((pMeiDev = (MEIX_Cntrl[entity])->MeiDevice[rel_ch]) != NULL) )
      {
         /* remove device */
         PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_WRN,
                ("MEI_DRV: iosDevAdd - device %s/%d" MEI_DRV_CRLF,
                  DRV_MEI_NAME, deviceNum));
         iosDevDelete((DEV_HDR *)pMeiDev);
      }
   }
...
#if (MEI_SUPPORT_IRQ == 1)
         {
            ...
            */
            ...
            if (pTmpXCntrl)
            {
               ...
               */
               if ( (ret = MEI_IfxIntConnect( ((VOIDFUNCPTR *) (pTmpXCntrl->IRQ_Num)),
                                                MEI_InterruptVxWorks,
               ...
               MEI_IntEnable_WrapVxW(pTmpXCntrl->IRQ_Num);
            }
         }
#else
         PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR,
...
/* ============================================================================
   Global I/O VINAX driver functions (vxWorks) - definition
   ========================================================================= */
...
int MEI_DevCreate(void)
{
   ...
   printf("(c) Copyright 2005, Infineon Technologies AG" MEI_DRV_CRLF MEI_DRV_CRLF);
   ...
   if (VINAXDrvNum <= 0)
   {
      ...
      VINAXDrvNum = iosDrvInstall(NULL, NULL,
                                 (FUNCPTR)MEI_CpeDevOpen,
                                 (FUNCPTR)MEI_Close,
                                 (FUNCPTR)MEI_Read,
                                 (FUNCPTR)MEI_Write,
                                 (FUNCPTR)MEI_IoCtl);
...
MEI_DEV_CREATE_ALLOC_ERROR:
   ...
   iosDrvRemove(VINAXDrvNum, IFX_TRUE);
   VINAXDrvNum = 0;
...
int MEI_DevDelete(void)
{
   /*
      Delete all devices from the IO system
   */
   MEI_DevDeleteAll(MEI_MAX_DFE_CHAN_DEVICES);
...
/* ============================================================================
   Setup VINAX driver interrupt functions (vxWorks) - wrapping
   ========================================================================= */

/**
   Set the wrapper function for the VxWorks intConnect function.
...
*/
int MEI_FctIntConnectSet(MEI_IntConnect_WrapVxW_t pIntConnectFct)
{
   ...
   MEI_IntConnect_WrapVxW = pIntConnectFct;
...
/**
   Set the wrapper function for the VxWorks intEnable function.
...
*/
int MEI_FctIntEnableSet(MEI_IntEnable_WrapVxW_t  pIntEnableFct)
{
   ...
   MEI_IntEnable_WrapVxW  = pIntEnableFct;
...
/**
   Set the wrapper function for the VxWorks intDisable function.
...
*/
int MEI_FctIntDisableSet(MEI_IntDisable_WrapVxW_t pIntDisableFct)
{
   ...
   MEI_IntDisable_WrapVxW = pIntDisableFct;
...
#endif /*  (MEI_EXPORT_INTERNAL_API == 1) */
