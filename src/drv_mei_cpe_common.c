...
MEIX_CNTRL_T *MEIX_Cntrl[MEI_MAX_DFEX_ENTITIES] = {0};
   ...
   if (entityNum >= MEI_MAX_DFEX_ENTITIES)
   {
      PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR,
             ("MEIX_DRV[--]: ERROR Cntrl Struct Allocate - invalid device num %d, max %d (-1)" MEI_DRV_CRLF,
              entityNum, MEI_MAX_DFEX_ENTITIES));
   ...
   */
   if ( (pXCntrl = MEIX_Cntrl[entityNum]) == NULL)
   {
      ...
      if ( MEI_MeiDevCntrlAlloc( pXCntrl,
                                   MEI_MAX_MEI_IF_PER_DEVICE) != IFX_SUCCESS )
      {
         MEI_MeiDevCntrlFree( pXCntrl,
                                MEI_MAX_MEI_IF_PER_DEVICE, 1);
...
IFX_int32_t MEI_DevXCntrlStructFree(IFX_uint8_t entity)
{
   ...
   {
      for (i=0; i<MEI_MAX_DFE_INSTANCE_PER_ENTITY; i++)
      {
      ...
      if ( i >= MEI_MAX_DFE_INSTANCE_PER_ENTITY )
      {
         /* no further devices on this chip control */
         MEIX_Cntrl[entity] = NULL;
         MEI_MeiDevCntrlFree(pXCntrl, MEI_MAX_DFE_INSTANCE_PER_ENTITY, 1);
         MEI_DRVOS_Free(pXCntrl);
         pXCntrl = IFX_NULL;
                                 ...
                                 IFX_int8_t        nLineNum)
{
   ...
   if (nLineNum > (MEI_MAX_DFE_CHAN_DEVICES -1))
   {
      PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR,
             ("MEI_DRV[--]: ERROR device setup - invalid line num %d max %d (-1)" MEI_DRV_CRLF,
              nLineNum, MEI_MAX_DFE_CHAN_DEVICES));
                              ...
                              MEI_FCT_CHECK_DEV_STATE  fctCheckDevState)
{
   ...
   {
      if ( (iMask*32) > MEI_MAX_DFE_CHAN_DEVICES)
         break;
      ...
      if (pVrxDevMask[iMask] != 0x0)
      {
         for (devFlag=0; devFlag<32; devFlag++)
         {
            ...
            if (fctCheckDevState)
            {
               ...
               if ( (entityNum < MEI_MAX_DFEX_ENTITIES) && (MEIX_Cntrl[entityNum] != NULL) &&
            ...
            if (iMask*32 +devFlag >= MEI_MAX_DFE_CHAN_DEVICES)
               break;
         }
      }
