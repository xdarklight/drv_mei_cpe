...
/* ============================================================================
   Description : VRX Driver, eCos part
   ========================================================================= */
...
#endif
#ifdef MEI_ECOS_PROC_FS
#include "drv_mei_cpe_ecos_proc.h"
#endif
...
/* ============================================================================
   I/O VRX driver functions (eCos) - declaration
   ========================================================================= */

MEI_STATIC IFX_long_t MEI_CpeDevOpen(
                              void              *priv,
...
MEI_STATIC void MEI_InterrupteCos(
                              int ISRParams);


/* ============================================================================
   I/O MEI X (Control) driver functions (eCos) - declaration
   ========================================================================= */

MEI_STATIC IFX_long_t MEI_DfeXDevOpen(
                                 void              *priv,
                                 const IFX_char_t  *pAnnex);
...
/* ============================================================================
   Local function (eCos) - declarations
   ========================================================================= */
...
/* ============================================================================
   VRX driver interrupt functions (eCos) - wrapping
   ========================================================================= */
#if (MEI_SUPPORT_IRQ == 1)
MEI_STATIC int MEI_IntSetupLocked = 0;

/* function ptr to the used intConnect function */
MEI_STATIC MEI_IntConnect_WrapECOS_t MEI_IntConnect_WrapECOS = IFX_NULL;

/* function ptr to the used Interrupt Enable Routine */
MEI_STATIC MEI_IntEnable_WrapECOS_t MEI_IntEnable_WrapECOS = IFX_NULL;

/* function ptr to the used Interrupt Enable Routine */
MEI_STATIC MEI_IntDisable_WrapECOS_t MEI_IntDisable_WrapECOS = IFX_NULL;
#endif

/* ============================================================================
   I/O driver functions (eCos) - definition
   ========================================================================= */
                                 ...
                                 const IFX_char_t *pAnnex)
{
   ...
   */
   return (IFX_long_t)pMeiDynCntrl;
}
...
MEI_STATIC IFX_int_t MEI_Poll(
                              void  *pprivate)
{
   MEI_DYN_CNTRL_T    *pDynCntrl  = (MEI_DYN_CNTRL_T*)pprivate;
   MEI_DEV_T          *pMeiDev  = (pDynCntrl) ? pDynCntrl->pMeiDev : IFX_NULL;
   MEI_DYN_NFC_DATA_T *pDynNfc    = (pDynCntrl) ? pDynCntrl->pInstDynNfc : IFX_NULL;

   if ( (pMeiDev == IFX_NULL) || (pDynCntrl == IFX_NULL) )
   {
      PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_NORMAL,
             ("MEI_DRV[-- --]: ERROR - missing line device data, poll failed" MEI_DRV_CRLF));
   ...
   if (pDynNfc == IFX_NULL)
   {
      PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_NORMAL,
             ("MEI_DRV[%02d-%02d]: NFC not enabled - poll failed" MEI_DRV_CRLF,
               MEI_DRV_LINENUM_GET(pMeiDev), pDynCntrl->openInstance));
...
/**
   ...
   - do the standard eCos intConnect
...
\remark
   The function wrapps the normal eCos intConnect function and additional
                                 ...
                                 int ISRParams)
{
   ...
   */
   if ( (ret = MEI_IntConnect_WrapECOS( pIntVector, pISRRoutine, ISRParams)) == -1 )
   {
...
*/
MEI_STATIC void MEI_InterrupteCos(int ISRParams)
{
   ...
   if (pMeiXCntrlList->IRQ_Protection <= 0)
   {
      ...
      PRN_ERR_INT_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR, (MEI_DRV_CRLF));
      MEI_IntDisable_WrapECOS(pMeiXCntrlList->IRQ_Num);
...
/* ============================================================================
   I/O VRX X (Control) driver functions (eCos) - definition
   ========================================================================= */
...
/* ============================================================================
   Local function definitions (eCos)
   ========================================================================= */
            ...
            */
            ...
            if (pTmpXCntrl)
            {
               ...
               */
               ...
               MEI_IntEnable_WrapECOS(pTmpXCntrl->IRQ_Num);
            }
         }
#else
         PRN_ERR_USR_NL( MEI_DRV, MEI_DRV_PRN_LEVEL_ERR,
...
/* ============================================================================
   Global I/O VRX driver functions (eCos) - definition
   ========================================================================= */
...
#if (MEI_SUPPORT_IRQ == 1)
/* ============================================================================
   Setup VRX driver interrupt functions (eCos) - wrapping
   ========================================================================= */

/**
   Set the wrapper function for the eCos intConnect function.
...
*/
int MEI_FctIntConnectSet(MEI_IntConnect_WrapECOS_t pIntConnectFct)
{
   ...
   MEI_IntConnect_WrapECOS = pIntConnectFct;
...
#if (MEI_SUPPORT_IRQ == 1)
/**
   Set the wrapper function for the eCos intEnable function.
...
*/
int MEI_FctIntEnableSet(MEI_IntEnable_WrapECOS_t  pIntEnableFct)
{
   ...
   MEI_IntEnable_WrapECOS  = pIntEnableFct;
...
#if (MEI_SUPPORT_IRQ == 1)
/**
   Set the wrapper function for the eCos intDisable function.
...
*/
int MEI_FctIntDisableSet(MEI_IntDisable_WrapECOS_t pIntDisableFct)
{
   ...
   MEI_IntDisable_WrapECOS = pIntDisableFct;
